...
| id | behavior | traces | kind | state | test |
| --- | --- | --- | --- | --- | --- |
| U1 | The naming produced by this controller's declared media type writes `ListResponse` with the names `tests/fixtures/pages/releases.json` carries, at every nesting level | FR-003, FR-006, SC-004 | example | PENDING | |
