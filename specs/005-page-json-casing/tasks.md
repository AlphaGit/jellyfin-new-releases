...
- [X] T011 Create `src/Jellyfin.Plugin.NewReleases/Api/PluginRoutes.cs` — `const string` members `Base = "Plugins/NewReleases"`, `Admin = Base + "/Admin"`, `UserView = Base + "/UserView"`, `UserViewAbsolute = "/" + UserView`, with an XML comment stating it is the single authoritative source (`FR-013`)
