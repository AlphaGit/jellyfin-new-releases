...
| A9  | A new `PluginConfiguration` has both sources enabled and exactly Album and EP included | US2-AS1 | example | PENDING | `tests/Jellyfin.Plugin.NewReleases.Tests/Configuration/PluginConfigurationTests.cs` |
| A10 | A fully changed configuration serialized with `XmlSerializer` and read back is equal field by field | US2-AS2 | example | PENDING | `Configuration/PluginConfigurationTests.cs` |
...
| id  | behavior | traces | kind | state | test |
| --- | --- | --- | --- | --- | --- |
| U1  | `Café Bleu` normalizes to `cafe bleu` (NFKC, diacritics stripped) | FR-005c | example | PENDING | `Matching/TitleNormalizerTests.cs` |
| U2  | `Rock & Roll` normalizes to `rock and roll` (case-fold, `&` → `and`) | FR-005c | example | PENDING | `Matching/TitleNormalizerTests.cs` |
| U3  | `A.B.  --  C!` normalizes to `ab c` (punctuation removed, whitespace collapsed, trimmed) | FR-005c | example | PENDING | `Matching/TitleNormalizerTests.cs` |
...
| U8  | `The Album` ≠ `Album` and `Vol. 2` ≠ `Volume 2` after normalization | FR-005c | example | PENDING | `Matching/TitleNormalizerTests.cs` |
...
| id  | behavior | traces | kind | state | test |
| --- | --- | --- | --- | --- | --- |
| U10 | MusicBrainz primary `Album`/`EP`/`Single` map to Album/EP/Single; `Broadcast` and `Other` map to `Other` | FR-004 | example | PENDING | `Matching/ReleaseTypeMapperTests.cs` |
| U11 | MusicBrainz secondaries `Compilation`/`Live`/`Remix`/`Soundtrack` map by name; `DJ-mix`, `Mixtape/Street`, `Demo`, `Audiobook` map to `Other` | FR-004 | example | PENDING | `Matching/ReleaseTypeMapperTests.cs` |
| U12 | Deezer `album`/`ep`/`single`/`compile` map to Album/EP/Single/Compilation as primary with no secondaries | FR-004 | example | PENDING | `Matching/ReleaseTypeMapperTests.cs` |
...
| id  | behavior | traces | kind | state | test |
| --- | --- | --- | --- | --- | --- |
| U18 | Database path is `{DataPath}/newreleases/newreleases.db` and the directory is created on first open | FR-006 | example | PENDING | `Storage/DatabaseTests.cs` |
...
| id  | behavior | traces | kind | state | test |
| --- | --- | --- | --- | --- | --- |
| U22 | Upserting an artist by `artist_key` keeps its `id` and updates name, mbid, `library_ids`, `album_count` | FR-001 | example | PENDING | `Storage/ArtistRepositoryTests.cs` |
...
| U36 | The list applies the enabled type set at read time: Album+Live is absent by default and present when Live is enabled, with `type=Live` | FR-004, R7 | example | PENDING | `Storage/ReleaseRepositoryTests.cs` |
| U37 | With `ReleasedSince=2020-01-01`, a release dated `2020-01-01` is returned, `2019-12-31` is not, and an undated release is returned | FR-003, R7 | example | PENDING | `Storage/ReleaseRepositoryTests.cs` |
| U38 | Filters: `artistId` narrows to one artist; `type` matches the display type; `state=Upcoming` returns rows with `date_sort > today` and not a row dated today; `Missing`/`Incomplete` exclude upcoming rows; `from`/`to` are inclusive and exclude undated rows | FR-008 | example | PENDING | `Storage/ReleaseRepositoryTests.cs` |
| U39 | `archived=false` omits rows that have a decision; `archived=true` returns only those, with kind and decided-at | FR-008, FR-016 | example | PENDING | `Storage/ReleaseRepositoryTests.cs` |
| U40 | The list returns at most 5 000 rows | R16 | example | PENDING | `Storage/ReleaseRepositoryTests.cs` |
| U41 | Edition upsert is unique on (source, source edition id); writing ownership stores state, method, library album id, compared edition id, missing tracks | FR-005, FR-005a | example | PENDING | `Storage/ReleaseRepositoryTests.cs` |
| U42 | Purge empties `release`, `source_entry`, `edition` and leaves `decision`, `library_artist`, `artist_source` rows in place | FR-013 | example | PENDING | `Storage/ReleaseRepositoryTests.cs` |
...
| id  | behavior | traces | kind | state | test |
| --- | --- | --- | --- | --- | --- |
| U44 | Setting a decision inserts it; setting again for the same key replaces kind, user and time | FR-016 | example | PENDING | `Storage/ArchiveRepositoryTests.cs` |
| U45 | Removing a decision deletes only that key; clearing deletes all | FR-013, FR-016 | example | PENDING | `Storage/ArchiveRepositoryTests.cs` |
...
| id  | behavior | traces | kind | state | test |
| --- | --- | --- | --- | --- | --- |
| U47 | Recording a call increments `calls_today`; when the stored day is not today (UTC) the counter restarts at 1 | FR-011 | example | PENDING | `Storage/SourceStateRepositoryTests.cs` |
...
| id  | behavior | traces | kind | state | test |
| --- | --- | --- | --- | --- | --- |
| U53 | `EnabledReleaseTypes()` is {Album, EP} by default and reflects each toggled flag | FR-004 | example | PENDING | `Configuration/PluginConfigurationTests.cs` |
...
| id  | behavior | traces | kind | state | test |
| --- | --- | --- | --- | --- | --- |
| U56 | Without a contact the User-Agent is `JellyfinNewReleases/<version>` | FR-018 | example | PENDING | `Sources/UserAgentBuilderTests.cs` |
...
| id  | behavior | traces | kind | state | test |
| --- | --- | --- | --- | --- | --- |
| U58 | Every request carries the User-Agent from `UserAgentBuilder` | FR-018 | example | PENDING | `Sources/SourceHttpClientTests.cs` |
| U59 | Each HTTP request records one call in `source_state` for that source | FR-011 | example | PENDING | `Sources/SourceHttpClientTests.cs` |
...
| id  | behavior | traces | kind | state | test |
| --- | --- | --- | --- | --- | --- |
| U68 | Album artists become library artists; an artist credited only as a featured track artist does not | FR-001 | example | PENDING | `Library/LibraryScannerTests.cs` |
| U69 | MBID is read from the artist's `MusicBrainzArtist`, else the album's `MusicBrainzAlbumArtist`; with neither the key is `name:<normalized name>` | FR-002 | example | PENDING | `Library/LibraryScannerTests.cs` |
...
| id  | behavior | traces | kind | state | test |
| --- | --- | --- | --- | --- | --- |
| U74 | An artist snapshot with an MBID is `Matched(mbid)` with zero HTTP requests | FR-002 | example | PENDING | `Sources/MusicBrainzSourceTests.cs` |
...
| id  | behavior | traces | kind | state | test |
| --- | --- | --- | --- | --- | --- |
| U85 | A candidate with the exact normalized name whose first albums page contains a library album title → `Matched(id)` | FR-002 | example | PENDING | `Sources/DeezerSourceTests.cs` |
...
| U97 | With a candidate and no stored editions the result asks for editions instead of deciding | FR-005, EC-7 | example | PENDING | `Matching/OwnershipMatcherTests.cs` |
